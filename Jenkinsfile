...
def deployWebsite(String environment) {
    withAWS(role: 'Jenkins', roleAccount: globalVariables.AWSAccountIDs('websites')) {
        sh "aws s3 cp dist/index.html s3://${BUCKETS[environment]} --cache-control no-store"
        sh "aws s3 sync dist s3://${BUCKETS[environment]} --sse AES256 --delete --cache-control ${CACHE[environment]}"
